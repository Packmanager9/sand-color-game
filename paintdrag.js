...
let pix = canvas_context.getImageData(0, 0, canvas.width, canvas.width)
...
let pix2 = canvas_context.getImageData(0, 0, canvas.width, canvas.width)
...
class Grain {
    constructor(t, k) {
        ...
        this.x = t
        this.y = k
        this.p = ((t) * 4) + (Math.floor(k * canvas.width) * 4)
        this.r = 128
        this.g = 128
        this.b = 128
        this.m = 1
    ...
    draw() {
        ...
        if (this.nin != -1) {
            if (this.n[this.nin].impact == 0) {
            if (sand.pixc < sand.pixy * 1.001) {
        ...
        if (this.impact == 0) {
            // if (this.nin != -1) {
                // console.log(this.nin, n)
                // if ((this.n[this.nin].r + this.n[this.nin].g + this.n[this.nin].b == 0)) {
                //     console.log((this.n[this.nin].r + this.n[this.nin].g + this.n[this.nin].b))
                //     // if(this.tapped == 0){
                //     this.r = 0
                //     this.g = 0
                //     this.b = 0
            ...
            if (this.nin != -1) {
                if (this.n[this.nin].impact == 0) {
                    // if(this.tapped == 0){
                    if ((this.n[this.nin].r + this.n[this.nin].g + this.n[this.nin].b < 64)) {
                        if (sand.pixc * 1.001 > sand.pixy) {
                            if (this.n[this.nin].nin != -1) {
                                this.r = 0
                                this.g = 0
                                this.b = 0
                                sand.pixc--
...
class Sandmap {
    constructor() {
        ...
        // canvas_context.fillRect(50, 50, 11, 11)
        canvas_context.fillRect(0, 0, 100, 100)
    ...
    draw() {
        ...
        this.tick++
        if (keysPressed[' ']) {
            canvas_context.drawImage(pomx, 0, 0, pomx.width, pomx.height, 0, 0, canvas.width, canvas.width)
            pix = canvas_context.getImageData(0, 0, canvas.width, canvas.width)
            this.pixc = 0
            for (let t = pix.data.length - 4; t >= 0; t -= 4) {
                if ((pix.data[t] + pix.data[t + 1] + pix.data[t + 2]) > 64) {
                    this.pixc++
                }
            }
            this.pixy = this.pixc
            ...
            for (let t = pix.data.length - 4; t >= 0; t -= 4) {
                ...
                if (keysPressed['e'] || true) {
                    ...
                    for (let g = 0; g < n.length; g++) {
                        lt.target = TIP_engine
                        lt.object = n[g]
                        let h = lt.hypotenuse()
                        if ((h < max && n[g].r + n[g].b + n[g].g < 1 && n[g].tapped != 1) && h < wideout) {
                            max = h
                            i = g
                        }
                    }
                ...
                if (n.length > 0) {
                    ...
                    if (nin > -1) {
                        if (this.blocks[x][y].m >= 1 && this.blocks[x][y].impact != 1) {
                            ...
                            if (n[nin].r + n[nin].g + n[nin].b < 64 && (pix.data[t] + pix.data[t + 1] + pix.data[t + 2]) > 64) {
                                n[nin].r = pix.data[t]
                                ...
                                if ((pix.data[t] + pix.data[t + 1] + pix.data[t + 2]) > 64 || n[nin].tapped == 1) {
                                    this.blocks[x][y].r = pix.data[t]
                                    this.blocks[x][y].g = pix.data[t + 1]
                                    this.blocks[x][y].b = pix.data[t + 2]
                                    if (nin > -1) {
        ...
        canvas_context.fillStyle = "black"
        canvas_context.fillRect(0, 0, 32, 20)
        canvas_context.fillStyle = "white"
        canvas_context.fillText(this.pixc, 10, 10)
    }
    neighbors(node) {
        var ret = [];
        var x = node.t;
