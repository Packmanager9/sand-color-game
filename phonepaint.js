...
let stop2 = 0
let frames = 0
function createHiPPICanvas(width, height, canvas_pass) {
    const ratio = window.devicePixelRatio;
    const canvas = canvas_pass
...
function setUp(canvas_pass, style = "#000000") {
    ...
    canvas = createHiPPICanvas(720 * .5, 720 * .5, canvas_pass)
    ...
    canvas2.addEventListener('pointerdown', e => {
        ...
        let p = new Point(XS_engine, YS_engine)
        if (sand.menu == 0) {

            if(sand.slideOn == 1){
                if(sand.slide.check(p)){
            ...
            for(let t = 0; t<sand.colorblocks.length;t++){

                if (sand.colorblocks[t].isPointInside(p)) {
                    // worldcolors[t] = {r:Math.random() *255, g:Math.random() *255, b:Math.random() *255}
                    sand.slideOn = 1
                    sand.slide = new Colorslide(sand.colorblocks[t], t)
                    return
                }

            }
            if (sand.buttons[0].isPointInside(p)) {
                sand.difficulty = 2
                sand.menu = 1
                return
            }
            if (sand.buttons[1].isPointInside(p)) {
                sand.difficulty = 3
                sand.menu = 1
                return
            }
            if (sand.buttons[2].isPointInside(p)) {
                sand.difficulty = 4
                sand.menu = 1
                return
            }
            if (sand.buttons[3].isPointInside(p)) {
                sand.difficulty = 5
                sand.menu = 1
                return
            }
            if (sand.buttons[5].isPointInside(p)) {
                sand.difficulty = 6
                sand.menu = 1
                return
            }
            if (sand.buttons[4].isPointInside(p)) {
                if (dynam == 4) {
                    dynam = 5
                    TIP_engine.x = (5 / 5) * 72
                    pix = canvas_context.getImageData(0, 0, canvas.width / dynam, canvas.width / dynam)

                    thash = {}
                    for (let t = 0; t < pix.data.length; t++) {

                        let x = ((t / 4)) % (canvas.width / dynam)
                        let y = Math.floor(((t / 4)) / (canvas.width / dynam))
                        thash[`${t}`] = { x: x, y: y }
                    }
                    sand = new Sandmap()
                    sand.buttons[4].color = "magenta"
                    return
                } else if (dynam == 10) {
                    sand.buttons[4].color = "#88ff00"
                    dynam = 4
                    TIP_engine.x = (4 / 5) * 72
                    pix = canvas_context.getImageData(0, 0, canvas.width / dynam, canvas.width / dynam)
                    ...
                    sand.buttons[4].color = "cyan"
                    dynam = 10
                    TIP_engine.x = (10 / 5) * 72
                    pix = canvas_context.getImageData(0, 0, canvas.width / dynam, canvas.width / dynam)
                    thash = {}
                    for (let t = 0; t < pix.data.length; t++) {
...
let pix = canvas_context.getImageData(0, 0, canvas.width / dynam, canvas.width / dynam)
...
canvas_context2.imageSmoothingEnabled = false

class Colorslide {
    constructor(box, t){
        this.t = t
        this.sliderRed = new Rectangle(box.x, box.y+42, 30, 100,  "red")
        this.sliderBlue = new Rectangle(box.x+80, box.y+42, 30, 100,  "blue")
        this.sliderGreen = new Rectangle(box.x+40, box.y+42, 30, 100,  "green")
    }
    draw(){

        let redslide = canvas_context2.createLinearGradient(this.sliderRed.x, this.sliderRed.y, +this.sliderRed.x+this.sliderRed.width, this.sliderRed.y+this.sliderRed.height);
        let blueslide  = canvas_context2.createLinearGradient(this.sliderBlue.x, this.sliderBlue.y, +this.sliderBlue.x+this.sliderBlue.width, this.sliderBlue.y+this.sliderBlue.height);
        let greenslide = canvas_context2.createLinearGradient(this.sliderGreen.x, this.sliderGreen.y, +this.sliderGreen.x +this.sliderGreen.width, this.sliderGreen.y+this.sliderGreen.height);

        redslide.addColorStop(0, `rgb(${0}, ${worldcolors[this.t].g}, ${worldcolors[this.t].b})`)
        redslide.addColorStop(1, `rgb(${255}, ${worldcolors[this.t].g}, ${worldcolors[this.t].b})`)


        greenslide.addColorStop(0, `rgb(${worldcolors[this.t].r}, ${0}, ${worldcolors[this.t].b})`)
        greenslide.addColorStop(1, `rgb(${worldcolors[this.t].r}, ${255}, ${worldcolors[this.t].b})`)


        blueslide.addColorStop(0, `rgb(${worldcolors[this.t].r}, ${worldcolors[this.t].g}, ${0})`)
        blueslide.addColorStop(1, `rgb(${worldcolors[this.t].r}, ${worldcolors[this.t].g}, ${255})`)

        canvas_context2.fillStyle = redslide
        canvas_context2.fillRect(this.sliderRed.x, this.sliderRed.y, this.sliderRed.width, this.sliderRed.height)
        canvas_context2.fillStyle = greenslide
        canvas_context2.fillRect(this.sliderGreen.x, this.sliderGreen.y, this.sliderGreen.width, this.sliderGreen.height)
        canvas_context2.fillStyle = blueslide
        canvas_context2.fillRect(this.sliderBlue.x, this.sliderBlue.y, this.sliderBlue.width, this.sliderBlue.height)

    }
    check(p){
        if(this.sliderRed.isPointInside(p)){
            worldcolors[this.t].r =Math.round(  (this.sliderRed.y-p.y)*2.55)*-1
            return true
        }
        if(this.sliderGreen.isPointInside(p)){
            worldcolors[this.t].g = Math.round(  (this.sliderGreen.y-p.y)*2.55)*-1
            return true
        }
        if(this.sliderBlue.isPointInside(p)){
            worldcolors[this.t].b = Math.round( (this.sliderBlue.y-p.y)*2.55)*-1
            return true
        }
        return true
    }
...
class Grain {
    ...
    draw() {
        ...
        if (this.type == -1) {
            if (this.redflag == 1) {
                this.r = worldcolors[0].r + ((Math.random()-.5) * 20)
                this.g = worldcolors[0].g + ((Math.random()-.5) * 20)
                this.b = worldcolors[0].b + ((Math.random()-.5) * 20)
                this.redflag = 0
                this.type = 0
            ...
            if (this.blueflag == 1) {
                this.r = worldcolors[1].r + ((Math.random()-.5) * 20)
                this.g = worldcolors[1].g + ((Math.random()-.5) * 20)
                this.b = worldcolors[1].b + ((Math.random()-.5) * 20)
                this.blueflag = 0
                this.type = 1
            }

            if (this.greenflag == 1) {
                this.r = worldcolors[2].r + ((Math.random()-.5) * 20)
                this.g = worldcolors[2].g + ((Math.random()-.5) * 20)
                this.b = worldcolors[2].b + ((Math.random()-.5) * 20)
                this.greenflag = 0
                this.type = 2
            }
            if (this.yellowflag == 1) {
                this.r = worldcolors[3].r + ((Math.random()-.5) * 20)
                this.g = worldcolors[3].g + ((Math.random()-.5) * 20)
                this.b = worldcolors[3].b + ((Math.random()-.5) * 20)
                this.yellowflag = 0
                
                this.type = 3
            }
            if (this.nextflag == 1) {
                this.r = worldcolors[4].r + ((Math.random()-.5) * 20)
                this.g = worldcolors[4].g + ((Math.random()-.5) * 20)
                this.b = worldcolors[4].b + ((Math.random()-.5) * 20)
                this.nextflag = 0
                this.type = 4
            }
            if (this.topflag == 1) {
                this.r = worldcolors[5].r + ((Math.random()-.5) * 20)
                this.g = worldcolors[5].g + ((Math.random()-.5) * 20)
                this.b = worldcolors[5].b + ((Math.random()-.5) * 20)
                this.topflag = 0
                this.type = 5
            }
        } else {
        ...
        if (this.flag == 1) {
            // if (this.r + this.g + this.b > 65) { //testing this
        ...
        if (this.k >= (canvas.width / dynam) - 2) {

            pix2.data[this.p] = 255
            pix2.data[this.p + 1] = 255
            pix2.data[this.p + 2] = 255
        }else{

        pix2.data[this.p] = this.r 
        pix2.data[this.p + 1] = this.g 
        pix2.data[this.p + 2] = this.b 
        }
        pix2.data[this.p + 3] = 255
        if (this.impact == 0) {
            if ((this.n[this.nin].black == 1)) {
                // if (this.r != 0) {
                // this.type = -1 //this.n[this.nin.type]
                this.zr = this.r
                // }
                // if (this.g != 0) {
                this.zg = this.g
                ...
                // this.n[this.nin].type = this.type
                // this.type = this.n[this.nin].type
                // this.type = -1 
                this.zr = this.r
                // }
                // if (this.g != 0) {
...
let worldcolors =  [{r:255,g:0,b:0},  {r:0,g:255,b:0},  {r:0,g:0,b:255},  {r:255,g:255,b:0},  {r:255,g:0,b:255},  {r:0,g:255,b:255}]

class Sandmap {
    constructor() {
        TIP_engine.x = 72 * (5/dynam)
        TIP_engine.y = 10
        this.slideOn = 0
        this.slide = {}
        this.menu = 0
        this.difficulty = 2
        this.buttons = [new Rectangle(60, 500, 100, 50, "white"), new Rectangle(180, 500, 100, 50, "#FFFFAA"), new Rectangle(300, 500, 100, 50, "yellow"), new Rectangle(420, 500, 100, 50, "orange"), new Rectangle(200, 600, 320, 50, "magenta"),new Rectangle(540, 500, 100, 50, "red")]
        // worldcolors = [{r:255,g:0,b:0},  {r:0,g:255,b:0},  {r:0,g:0,b:255},  {r:255,g:255,b:0},  {r:255,g:0,b:255},  {r:0,g:255,b:255}]
        this.colorblocks = [new Rectangle(100, 40, 40, 40, `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`), new Rectangle(220, 40, 40, 40, `rgb(${worldcolors[1].r}, ${worldcolors[1].g}, ${worldcolors[1].b})`), new Rectangle(340, 40, 40, 40, `rgb(${worldcolors[2].r}, ${worldcolors[2].g}, ${worldcolors[2].b})`), new Rectangle(460, 40, 40, 40, `rgb(${worldcolors[3].r}, ${worldcolors[3].g}, ${worldcolors[3].b})`), new Rectangle(580, 40, 40, 40, `rgb(${worldcolors[4].r}, ${worldcolors[4].g}, ${worldcolors[4].b})`), new Rectangle(600, 40, 40, 40, `rgb(${worldcolors[5].r}, ${worldcolors[5].g}, ${worldcolors[5].b})`), ]
        this.show = "#FFFFFF"
        this.blocks = []
        for (let t = 0; t < (canvas.width / dynam); t++) {
            this.blocks.push([])
            for (let k = 0; k < (canvas.width / dynam); k++) {
    ...
    redcheck(point) {
        ...
        for (let t = -7; t < 8; t++) {
            for (let k = -7; k < 8; k++) {
                this.blocks[Math.abs((t + x) % (canvas.width / dynam))][Math.abs((k + y) % (canvas.width / dynam))].redflag = 1
            }
        }
    ...
    greencheck(point) {
        ...
        for (let t = -7; t < 8; t++) {
            for (let k = -7; k < 8; k++) {
                this.blocks[Math.abs((t + x) % (canvas.width / dynam))][Math.abs((k + y) % (canvas.width / dynam))].greenflag = 1
            }
        }
    }
    yellowcheck(point) {
        ...
        for (let t = -7; t < 8; t++) {
            for (let k = -7; k < 8; k++) {
                this.blocks[Math.abs((t + x) % (canvas.width / dynam))][Math.abs((k + y) % (canvas.width / dynam))].yellowflag = 1
            }
        }
    }
    nextcheck(point) {
        ...
        for (let t = -7; t < 8; t++) {
            for (let k = -7; k < 8; k++) {
                this.blocks[Math.abs((t + x) % (canvas.width / dynam))][Math.abs((k + y) % (canvas.width / dynam))].nextflag = 1
            }
        }
    }
    topcheck(point) {
        ...
        for (let t = -7; t < 8; t++) {
            for (let k = -7; k < 8; k++) {
                this.blocks[Math.abs((t + x) % (canvas.width / dynam))][Math.abs((k + y) % (canvas.width / dynam))].topflag = 1
            }
        }
    }
    bluecheck(point) {
        ...
        for (let t = -7; t < 8; t++) {
            for (let k = -7; k < 8; k++) {
                ...
                this.blocks[Math.abs((t + x) % (canvas.width / dynam))][Math.abs((k + y) % (canvas.width / dynam))].blueflag = 1
            }
        }
    ...
    checkkill(point, target) {
        ...
        n.push(er)
        let f = [...n]
        for (let q = 0; q < ((canvas.width / dynam) + 120); q++) { //loops for thw apparent width AND 120 more layers (this is bad)
        ...
        if (strip.length > ((canvas.width / dynam) - 1)) { //close enough to the width, doesn't miss much
            for (let d = 0; d < n.length; d++) {
    ...
    draw() {
        ...
        this.colorblocks = [new Rectangle(80, 40, 40, 40, `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`), new Rectangle(180, 40, 40, 40, `rgb(${worldcolors[1].r}, ${worldcolors[1].g}, ${worldcolors[1].b})`), new Rectangle(280, 40, 40, 40, `rgb(${worldcolors[2].r}, ${worldcolors[2].g}, ${worldcolors[2].b})`), new Rectangle(380, 40, 40, 40, `rgb(${worldcolors[3].r}, ${worldcolors[3].g}, ${worldcolors[3].b})`), new Rectangle(480, 40, 40, 40, `rgb(${worldcolors[4].r}, ${worldcolors[4].g}, ${worldcolors[4].b})`), new Rectangle(580, 40, 40, 40, `rgb(${worldcolors[5].r}, ${worldcolors[5].g}, ${worldcolors[5].b})`), ]
    


        if (this.menu == 0) {
            canvas_context2.clearRect(0, 0, canvas.width, canvas.width)
            canvas_context2.drawImage(title, 0, 20)
            if(this.slideOn == 1){
                this.slide.draw()
            }
            this.buttons[0].draw()
            ...
            this.buttons[3].draw()
            this.buttons[5].draw()
            for(let t = 0; t<this.colorblocks.length;t++){
                this.colorblocks[t].draw()
            }

            canvas_context2.fillStyle = "white"
            canvas_context2.font = "18px arial"

            canvas_context2.fillText("Set Colors:", 10, 20)

            canvas_context2.fillStyle = "Black"
            canvas_context2.font = "18px arial"

            if (dynam == 4) {
                this.buttons[4].color = "#AAFF00"
                canvas_context2.fillText("Scale: Huge", this.buttons[4].x + 100, this.buttons[4].y + 25)
            }
            if (dynam == 5) {
                this.buttons[4].color = "magenta"
                canvas_context2.fillText("Scale: Medium", this.buttons[4].x + 100, this.buttons[4].y + 25)
            }
            if (dynam == 10) {
                this.buttons[4].color = "cyan"
                canvas_context2.fillText("Scale: Small", this.buttons[4].x + 100, this.buttons[4].y + 25)
            }
            this.buttons[4].draw()
            ...
            canvas_context2.fillText("Easy (2)", this.buttons[0].x + 10, this.buttons[0].y + 25)
            canvas_context2.fillText("Normal (3)", this.buttons[1].x + 10, this.buttons[1].y + 25)
            canvas_context2.fillText("Hard (4)", this.buttons[2].x + 10, this.buttons[2].y + 25)
            canvas_context2.fillText("Insane (5)", this.buttons[3].x + 10, this.buttons[3].y + 25)
            canvas_context2.fillText("Absurd (6)", this.buttons[5].x + 10, this.buttons[5].y + 25)
            if (dynam == 4) {
                this.buttons[4].color = "#AAFF00"
                canvas_context2.fillText("Scale: Huge", this.buttons[4].x + 100, this.buttons[4].y + 25)
            }
        ...
        this.donecheck++
        if (this.first < -10) {
            canvas_context.drawImage(pomx, 0, 0, pomx.width, pomx.height, 0, 0, canvas.width / dynam, canvas.width / dynam)
            this.first++
        } else {

            canvas_context.drawImage(canvas, 0, 0, canvas.width / 1, canvas.width / 1, 0, 0, canvas.width / dynam, canvas.width / dynam)
        }
        ...
        //     link.draw()
        //     link2.draw()
        //     canvas_context.drawImage(canvas2, 0, 0, canvas.width * dynam, canvas.width * dynam, 0, 0, canvas.width, canvas.width)
        //     for (let t = 0; t < 5; t++) {
        //         let spark = new Rectangle((TIP_engine.x * dynam) + ((Math.random() - .5) * 25), (TIP_engine.y * dynam) + ((Math.random() - .5) * 25), 4, 4, "white")
        //         let spark2 = new Rectangle((TIP_engine.x * dynam) + ((Math.random() - .5) * 15), (TIP_engine.y * dynam) + ((Math.random() - .5) * 15), 2, 2, "cyan")
        //         spark.draw()
        //         spark2.draw()
        //     }
        // }
        ...
        if (this.done > 0) {
            ...
            canvas_context2.fillRect(((TIP_engine.x) * dynam) - 8, ((TIP_engine.y) * dynam) - 8, 16, 16)
            canvas_context2.fillStyle = "black"
        ...
        if (this.wasd != 1) {
            ...
            canvas_context2.fillText("Or tap/click!!", 230, 300)
            if (keysPressed['a']) {
                this.wasd = 1
            }
            if (keysPressed['d']) {
        ...
        //     canvas_context2.fillStyle = "#66ff00"
        // }else {
        //     canvas_context2.fillStyle  ="#FFffff"

        // }
        canvas_context2.fillRect(((TIP_engine.x) * dynam) - 5, ((TIP_engine.y) * dynam) - 5, 10, 10)
            ...
            for (let t = pix.data.length - 4; t >= 0; t -= 4) {
                ...
                if (n.length > 0) {
                    ...
                    if (this.blocks[x][y].m >= 1 && this.blocks[x][y].impact != 1) {
                        ...
                        if ((n[nin].r + n[nin].g + n[nin].b) < 1 && pix.data[t] + pix.data[t + 1] + pix.data[t + 2] > 0 && n[nin].tapped != 1) {
                            n[nin].r = pix.data[t]
                            n[nin].g = pix.data[t + 1]
                            n[nin].b = pix.data[t + 2]

                    let l =  n[nin].type
                    n[nin].type = this.blocks[x][y].type
                    this.blocks[x][y].type = l

                            this.blocks[x][y].nin = nin
                            // this.blocks[x][y].type = n[nin].type
                            n[nin].tapped = 1
                            n[nin].change = 1
                            // n[nin].type = this.blocks[x][y].type
        ...
        if (this.yellowFlag == 1) {
            this.yellowcheck(TIP_engine)
            this.yellowFlag = 0
        }
        if (this.nextFlag == 1) {
            this.nextcheck(TIP_engine)
            this.nextFlag = 0
        }
        if (this.topFlag == 1) {
            this.topcheck(TIP_engine)
            this.topFlag = 0
        }


        let link = new LineOP(new Point(canvas.width, canvas.width), new Point(TIP_engine.x * dynam, TIP_engine.y * dynam), "cyan", 3)
        let link2 = new LineOP(new Point(canvas.width, canvas.width), new Point(TIP_engine.x * dynam, TIP_engine.y * dynam), "white", 2)
        // if (this.ucheckFlag == 1) {
        if (keysPressed[' ']) {
        ...
        if (this.donecheck % 1 == 0 && this.donecheck > 10) {
            ...
            for (let t = 0; t < this.blocks.length; t++) {
                for (let k = 0; k < this.blocks.length; k++) {
                    if (this.blocks[t][k].r + this.blocks[t][k].g + this.blocks[t][k].b > 128) {
                        count++
                    } else { }
                }
            }
            count /= ((canvas.width / dynam) * (canvas.width / dynam))
            if (count > .875) {
                this.done = 1
            }
        }
        ...
        canvas_context.drawImage(canvas, 0, 0, canvas.width / dynam, canvas.width / dynam, 0, 0, canvas.width, canvas.width)
    ...
    hueneighbors(node) {
        ...
        if (grid[x - 1] && grid[x - 1][y]) {
            ...
            let hue = Math.abs(((f.zr - node.zr) * (f.zr - node.zr)) + ((f.zg - node.zg) * (f.zg - node.zg)) + ((f.zb - node.zb) * (f.zb - node.zb)))
            // ////console.log(f)
            if(f.type == node.type){//if (hue < (60 * 60)) {
                ret.push(grid[x - 1][y]);
            }
            // }
            // }
...
function main() {
    ...
    if (keysPressed[' ']) {
        ...
        if (TIP_engine.x < 0) {
            TIP_engine.x = (canvas.width / dynam) - 5
        }
        if (TIP_engine.x > canvas.width / dynam) {
            TIP_engine.x = 5
        }
    }
    TIP_engine.y = 10
    ...
    if (sandtick % 40 === 0) {
        // if(sandtick%40 === 0){
        let p = new Point(0, 0)
        let color = { r: 0, g: 0, b: 0, zr: 0, zg: 0, zb: 0, type:-1 }
        for (let t = 0; t < (canvas.width / dynam) - 3; t++) {
            if (color.type == sand.blocks[p.x][p.y].type) {
                p.y++
                continue
            }
            sand.checkkill(p, sand.blocks[(canvas.width / dynam) - 1][t])
            color.zr = sand.blocks[p.x][p.y].zr
            color.zg = sand.blocks[p.x][p.y].zg
            color.zb = sand.blocks[p.x][p.y].zb
            color.type = sand.blocks[p.x][p.y].type
            color.r = sand.blocks[p.x][p.y].zr
            color.g = sand.blocks[p.x][p.y].zg
            color.b = sand.blocks[p.x][p.y].zb
            if((sand.blocks[p.x][p.y].type == -1)){
                color = { r: 0, g: 0, b: 0, zr: 0, zg: 0, zb: 0, type:-1 }
            }
            p.y++
        }
        ...
        if (sand.difficulty == 2) {
            if (sand.show ==  `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`) {
                sand.redFlag = 1
            }
            if (sand.show == `rgb(${worldcolors[1].r}, ${worldcolors[1].g}, ${worldcolors[1].b})`) {
                sand.blueFlag = 1
            }
            if (Math.random() < .5) {
                sand.show = `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`
            } else {
                sand.show = `rgb(${worldcolors[1].r}, ${worldcolors[1].g}, ${worldcolors[1].b})`
            }
        }

        if (sand.difficulty == 3) {
            if (sand.show ==  `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`) {
                sand.redFlag = 1
            }
            if (sand.show == `rgb(${worldcolors[1].r}, ${worldcolors[1].g}, ${worldcolors[1].b})`) {
            ...
            if (sand.show == `rgb(${worldcolors[2].r}, ${worldcolors[2].g}, ${worldcolors[2].b})`) {
                sand.greenFlag = 1
            }
            if (Math.random() < .33333) {
                ...
                sand.show = `rgb(${worldcolors[2].r}, ${worldcolors[2].g}, ${worldcolors[2].b})`
            }
        }
        if (sand.difficulty == 4) {
            ...
            if (sand.show == `rgb(${worldcolors[3].r}, ${worldcolors[3].g}, ${worldcolors[3].b})`) {
                sand.yellowFlag = 1
            }
            if (Math.random() < .25) {
                sand.show = `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`
                ...
                sand.show = `rgb(${worldcolors[3].r}, ${worldcolors[3].g}, ${worldcolors[3].b})`
            }
        }

        if (sand.difficulty == 5) {
            if (sand.show ==  `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`) {
                sand.redFlag = 1
            }
            if (sand.show == `rgb(${worldcolors[1].r}, ${worldcolors[1].g}, ${worldcolors[1].b})`) {
            ...
            if (sand.show ==  `rgb(${worldcolors[4].r}, ${worldcolors[4].g}, ${worldcolors[4].b})`) {
                sand.nextFlag = 1
            }
            if (Math.random() < .2) {
                sand.show = `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`
                ...
                sand.show = `rgb(${worldcolors[4].r}, ${worldcolors[4].g}, ${worldcolors[4].b})`
            }
        }
        if (sand.difficulty == 6) {
            ...
            if (sand.show ==  `rgb(${worldcolors[5].r}, ${worldcolors[5].g}, ${worldcolors[5].b})`) {
                sand.topFlag = 1
            }
            if (Math.random() < .1875) {
                sand.show = `rgb(${worldcolors[0].r}, ${worldcolors[0].g}, ${worldcolors[0].b})`
                ...
                sand.show = `rgb(${worldcolors[5].r}, ${worldcolors[5].g}, ${worldcolors[5].b})`
            }
        }
    }
    ...
    //     recording = 0
    //     video_recorder.stop()
    //     video_recorder.download('sand.webm')
    // }
